...
**仓库地址**：<https://github.com/SiqYin/SYSite>　｜　**线上地址**：<https://siqyin.github.io/SYSite/>
...
| CSS | 8 KB | 首屏 |
| JS | 11 KB | 首屏 |
| 主字体（霞鹜文楷首屏子集） | 347 KB | 首屏 |
| **首屏合计** | **约 400 KB** | |
| `player-data.json`（直链＋歌词） | 175 KB（gzip 89 KB） | **点播放时才取** |
| 歌词／检索字体 | 464 KB | **打开播放器或搜索时才下载** |
| 其余全部汉字（13 片） | 7.1 MB | **只有真渲染到「前两层没有的字」时才下对应那一片** |

字体分三层（`src/styles/fonts.css` 由 `scripts/build_font.py` 生成）：

| 层 | 覆盖 | 何时下载 |
|---|---|---|
| Content | 首屏会出现的字（页面 + 四语文案 + BGM 曲名） | 首屏，347 KB |
| Lyrics | 歌词 + 站内检索索引里的字 | 打开播放器 / 用搜索时 |
| Ext（13 片） | 霞鹜文楷里其余**全部**汉字（基本区 20992 + 扩展A 6592 + 兼容 + 假名 + 标点） | 按 `unicode-range` 按需，通常永远用不到 |

这样全站所有汉字都有霞鹜文楷字形（不会再出现「一部分汉字套了霞鹜文楷、一部分没套」），
而首屏仍然只下 Content 那一份。想省掉歌词层的话，把 `config/site.json` 的
`player.lyricFont` 设为 `false`（代价：歌词里的生僻字会落到 Ext 分片，按需多下一次）。
...
霞鹜文楷 Regular（25 MB，4.6 万码位）在构建期切成三层 woff2：

1. **Content**：首屏会出现的字 —— 扫描 `dist/` 的 HTML/CSS/JS + 四语文案 + BGM 曲名。
   注意要解 HTML 实体（`&#9654;` = ▶ 之类），也要把所有非 ASCII 符号算进来，
   否则浏览器会为这些字往下一层回退、白下字体。
2. **Lyrics**：歌词 + `search-index`。带 `unicode-range`，字符不在范围内时浏览器不必下载就能跳过。
3. **Ext（13 片）**：霞鹜文楷里其余全部汉字，按码位连续分片并声明 `unicode-range`。
   全量汉字都随仓库发出，所以新投稿的新字、歌词里的方言/生僻字都不会缺字；
   而浏览器只会下载「真的渲染到了」的那一片。

字体包由 workflow 下载到 `.cache/fonts/` 并缓存（首次 80 MB，之后命中缓存）。
拿不到字体包时 `build_font.py` 会跳过，仓库里已提交的字体与 `fonts.css` 保持不变。

## 定时自动更新

完全由 **GitHub Actions** 负责（`.github/workflows/update.yml`）：
每 20 分钟（UTC cron `3,23,43 * * * *`，音频直链带签名、约 30 分钟失效）跑一遍
